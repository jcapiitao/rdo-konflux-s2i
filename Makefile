...
pip-compile-runtime-deps: build-dev-libs-image
	podman run --rm \
		...
		-w /src/workspace \
		localhost/s2i-cs10-dev-libs:latest \
		sh -c "pip-compile requirements.in \
		       -c https://raw.githubusercontent.com/openstack/requirements/refs/heads/master/upper-constraints.txt \
		       --generate-hashes \
		       --allow-unsafe \
...
pip-compile-buildtime-deps: build-dev-libs-image
	...
	podman run --rm \
		...
		-w /src/workspace \
		localhost/s2i-cs10-dev-libs:latest \
		sh -c "pip-compile requirements-build.in \
		       requirements-build-added-manually.in \
		       -c https://raw.githubusercontent.com/openstack/requirements/refs/heads/master/upper-constraints.txt \
...
pip-find-buildtime-deps: build-dev-libs-image
	podman run --rm \
		...
		-w /src/workspace \
		localhost/s2i-cs10-dev-libs:latest \
		sh -c "pip_find_builddeps requirements.txt \
		       --append \
